...
# Test drop-off percentages, edge cases (visitor enters and exits immediately),
# and the re-entry deduplication logic."
#
# CHANGES MADE:
# - Added test for visitor who enters but never visits a zone
# - Added test for re-entry in funnel (same visitor re-entering)
# - Verified drop-off % calculations with known data
# - Added test for multiple visitors with different journey depths
...
class TestFunnelDropOff:
    ...
    async def test_drop_off_percentages(self, client):
        ...
        for vid in ["VIS_A", "VIS_B"]:
            events.append(make_event("ZONE_ENTER", visitor_id=vid,
                                     zone_id="SKINCARE",
                                     timestamp="2026-04-10T18:01:00Z"))
        
        # Only A reaches billing
        events.append(make_event("ZONE_ENTER", visitor_id="VIS_A",
                                 zone_id="BILLING",
